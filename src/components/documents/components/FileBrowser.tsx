...
const sampleFolders: DocumentType[] = Array.from(Array(10)).map((_, i) => {
  ...
  return {
    ...
    children: i === 0 ? ['1folder', '2folder', '0file'] : i === 1 ? ['3folder', '4folder', '5folder', '6folder', '7folder', '8folder', '9folder', '1file', '2file', '3file', '4file', '5file', '6file'] : null,
    type: 'folder'
  }
});
...
const FileBrowser = ({ height, width, bgColor, border, borderRadius, browserDocuments }: FileBrowserProps) => {
  ...
  React.useEffect(() => {
    setBrowserHeight(window.innerHeight * .96)
    setBrowserWidth(window.innerWidth * .96)
