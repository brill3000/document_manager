...
interface ActionMenuProps {
    ...
    handleMenuClick: (e: React.MouseEvent<HTMLLIElement, MouseEvent>, type: 'new_folder' | 'paste' | 'paste_all' | 'edit' | 'delete') => void
}
    ...
    return (
        ...
        >
            <MenuItem
                ...
                onClick={(e) => { setSelected('new_folder'); handleMenuClick(e, 'new_folder') }}
            >
                <Stack height='max-content' direction='row' spacing={1} p={.3} borderRadius={1}>
                    <BsFolderPlus size={18} />
                    <Typography variant='body2' fontSize={12} color={(theme) => theme.palette.text.primary} noWrap>New Folder</Typography>
                </Stack>
            </MenuItem>
            <MenuItem
                selected={selected === 'paste'}
                onClick={(e) => { setSelected('paste'); handleMenuClick(e, 'paste') }}
            >
                <Stack height='max-content' direction='row' spacing={1} p={.3} borderRadius={1}>
                    <BiBrush size={20} />
                    <Typography variant='body2' fontSize={12} color={(theme) => theme.palette.text.primary} noWrap>Paste</Typography>
                </Stack>
            </MenuItem>
            <MenuItem
                selected={selected === 'paste_all'}
                onClick={(e) => { setSelected('paste_all'); handleMenuClick(e, 'paste_all') }}
            >
                <Stack height='max-content' direction='row' spacing={1} p={.3} borderRadius={1}>
                    <BiPaste size={20} />
                    <Typography variant='body2' fontSize={12} color={(theme) => theme.palette.text.primary} noWrap>Paste All</Typography>
                </Stack>
            </MenuItem>
