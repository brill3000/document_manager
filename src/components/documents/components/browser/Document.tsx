        ...
        document.is_dir ?
          ...
          >
            <Stack justifyContent='center' alignItems='center' spacing={1} ref={drag} display={isDragging ? 'none' : 'flex'}>
              <Box
                borderRadius={2}
                {...(isSelected || isOver ? { bgcolor: grey[300] } : isHovered ? { bgcolor: alpha(grey[300], .2) } : {})}
                width='max-content'
                height='max-content'
                onClick={handleClick}
              ...
              <Box
                ...
                px={isRenaming ? 0 : 1}
                py={isRenaming ? 0 : 1}
                bgcolor={theme => (isSelected || isOver) && !isRenaming ? theme.palette.primary.main : isHovered && !isRenaming ? (alpha(theme.palette.primary.main, .1)) : '#f9f7f6'}
                width={isRenaming ? '100%' : 115}
                height='max-content'
                border={isRenaming ? 0 : .3}
              ...
              >
                ...
                {
                  isRenaming ?
                    <RenameDocument selected={selected} renameFn={renameFn} renameTarget={renameTarget} disableDoubleClick={disableDoubleClickFn} />
                    :
                    <Box sx={{
                      overflow: 'hidden',
          ...
          <Stack justifyContent='center' alignItems='center' spacing={1} ref={drag} display={isDragging ? 'none' : 'flex'}>
            <Box
              borderRadius={2}
              {...(isSelected ? { bgcolor: grey[300] } : isHovered ? { bgcolor: alpha(grey[300], .2) } : {})}
              width='max-content'
              height='max-content'
              pt={2}
              pb={.5}
            ...
            >
              {fileIcon(document.type, browserHeight * .1, browserHeight * .005)}
            </Box>
            <Box
              borderRadius={1}
              px={isRenaming ? 0 : 1}
              py={isRenaming ? 0 : 1}
              bgcolor={(isSelected || isOver) && !isRenaming ? blue[600] : isHovered && !isRenaming ? (alpha(blue[100], .4)) : (alpha(blue[100], 0))}
              width={isRenaming ? '100%' : 115}
              height='max-content'
              border={isRenaming ? 0 : .3}
              borderColor={(theme) => theme.palette.divider}
