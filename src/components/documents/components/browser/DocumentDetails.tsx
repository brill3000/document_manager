  ...
  return (
    <List
      sx={{
        ...
        position: 'relative',
        overflow: 'auto',
        maxHeight: browserHeight !== 0 && browserHeight !== undefined ?  browserHeight * .7 * .8 : '50%',
        '& ul': { padding: 1 },
      }}
      subheader={<li />}
    >
      {[{ title: "General Infomation" }, { title: 'Permissions & Access' }, { title: 'tags' }].map((sectionId, i) => (
        <li key={`section-${sectionId.title}`}>
          <ul style={{ minHeight: i === 2 ? browserHeight !== 0 && browserHeight !== undefined ?  browserHeight * .7 * .4 : '40%': 'max-content' }}>
