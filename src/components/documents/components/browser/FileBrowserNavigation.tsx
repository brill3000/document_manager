...
const FolderBrowserNavigation = React.forwardRef<HTMLInputElement, FileBrowserNaviagationProps>(function FolderBrowserNavigation (props, ref) {
    ...
    return (
        ...
        >
            <Grid item xs={11}>
                ...
                >
                    {
                        Array.isArray(navHistory) && navHistory.filter((_: string | number, i: number) => i !== navHistory.length - 1 && i > 0).map((hist: string | number, index: number) =>
                            <RefactoredMenuItem
                                key={hist}
                ...
                </StyledMenu>
                {
                    Array.isArray(navHistory) ?
                        <Breadcrumbs aria-label="breadcrumb">
                            ...
                            {
                                [...navHistory].filter((_, i) => i > 0).map((element, i: number) => {
                                    return (
                                        navHistory.length < 4 ?
                                            <CustomChip
                                                key={element}
...
function RefactoredMenuItem({ handleClose, handleClick, hist, index, doc }: RefactoredMenuItemInterface) {
    ...
    return (
        <MenuItem
            ref={drop}
            onClick={(e) => { handleClose(); handleClick(e, hist); }}
            key={index}
            sx={{
        ...
        >
            <Stack height='max-content' direction='row' spacing={1} p={.3} borderRadius={1}>
                ...
                <Typography variant='body2' fontSize={12} color={(theme) => theme.palette.text.primary} noWrap>{doc?.doc_name}</Typography>
            </Stack>
        </MenuItem>
    );
}
