...
export const useModel = (root: string | number | null | undefined, documents: DocumentType[]): UseModelExports => {
    ...
    */
     ...
     * @returns {boolean}
     * */
    const has = React.useCallback((key: string | number): boolean => {
        return map.has(key)
    }, [map])
    /**
     * Create a new folder
     ...
     * @returns {boolean}
     */
    const uploadFiles = React.useCallback((parent: string | number, files: DocumentType[]): boolean | Error => {
        try {
            if (!parent && !map.has(parent)) throw new Error('The folder you are uploading appears to have been deleted, reload to check for changes')
            const parentDoc = map.get(parent);
            if (parentDoc === undefined) throw new Error('The folder you are uploading appears to have been deleted, reload to check for changes')
 ...
 * @returns {boolean}
 */
    const createFolder = React.useCallback((parent: string | number, folder: DocumentType): boolean | Error => {
        try {
            if (!parent && !map.has(parent)) throw new Error('The folder you are uploading appears to have been deleted, reload to check for changes')
            const parentDoc = map.get(parent);
            if (parentDoc === undefined) throw new Error('The folder you are uploading appears to have been deleted, reload to check for changes')
    ...
    const getChildren = React.useCallback((parent: string | number | null): Array<DocumentType | undefined> | null => {
        if (parent && has(parent)) {
            const children = map.get(parent)?.children
            if (Array.isArray(children) && children.length > 0) {
                return children.filter(x => map.has(x)).map(child => {
    ...
    const changeDetails = React.useCallback((key: string | number, details: EditDocumentType): boolean | Error => {
    ...
    const move = React.useCallback((key: string | number | null, parent: string | number | null): boolean | Error => {
        try {
            ...
            if (Array.isArray(doc.children) && doc.children.includes(parent)) return new Error('Cannot move a parent folder into a child folder')
    ...
    const deleteById = React.useCallback((key: string | number): boolean | Error => {
        try {
            let deleted = false
            ...
            if (parent !== null && parent !== undefined) {
                ...
                throw new Error('Could not delete from document from parent document, maybe the parent folder has already been deleted')
            }
            if (copy.has(key)) {
