    ...
    actions: {
        ...
        getChildren: (parent: key | null) => {
            if (parent && get().fileMap.has(parent)) {
                ...
                if (Array.isArray(children) && children.length > 0) {
                    return children.filter(x => get().fileMap.has(x)).map(child => {
        ...
        move: (key: string | number | null, target: string | number | null): boolean | Error => {
            try {
                ...
                if (Array.isArray(doc.children) && doc.children.includes(target)) throw new Error('Cannot move a parent folder into a child folder')
                const prevParent = doc.parent;
                if (prevParent === target) throw new Error('You cannot move a document to the same location');
                if (prevParent === null || prevParent === undefined) throw new Error('The Folder you are moving from may have been deleted, Reload to view this');
        ...
        uploadFiles: (parent: string | number, files: DocumentType[]): boolean | Error => {
            try {
                const mapCopy = new Map(get().fileMap)
                if (parent === null || parent === undefined || !mapCopy.has(parent)) throw new Error('The folder you are uploading appears to have been deleted, reload to check for changes')
