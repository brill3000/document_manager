...
function DraggableBox({ id, showEmptyPlaceholder, item }) {
    ...
    return (
        <div ref={drag} className="dragitem" style={getDraggableBoxStyles(left, top, isDragging)}>
            {title}
        </div>
    );
}
        ...
        <>
            <div ref={drop} className="dragcontainer">
                {Object.keys(boxes).map((key) => (
                    <DraggableBox key={key} id={key} item={boxes[key]} showEmptyPlaceholder={showEmptyPlaceholder} />
                ))}
            </div>
