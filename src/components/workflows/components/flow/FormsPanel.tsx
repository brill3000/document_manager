...
import { FormikCheckbox, FormikText } from 'global/UI/FormMUI/Components';
import { uuidv4 } from '@firebase/util';
import { FormsCard } from './CreateFlow';

export function FormsPanel(newFormTitle: string, handleNewFormTitleChange: React.ChangeEventHandler<HTMLInputElement>, openEditIndex: { input: boolean; large_input: boolean; checkbox: boolean; radio: boolean; select: boolean; submit: boolean; }, setOpenEditIndex: React.Dispatch<React.SetStateAction<{ input: boolean; large_input: boolean; checkbox: boolean; radio: boolean; select: boolean; submit: boolean; }>>, setEditDetails: React.Dispatch<React.SetStateAction<Array<any>>>, addFormComponent: (type: string, label: string, uid: string, placeholder?: string, minRows?: number, isRequired?: boolean | string, initialValue?: string, defaultChecked?: boolean) => void, formComponents: any[], savedForms: any[], setSavedForms: React.Dispatch<React.SetStateAction<any[]>>, setFormComponents: React.Dispatch<React.SetStateAction<any[]>>, setNewFormTitle: React.Dispatch<React.SetStateAction<string>>, editDetails: any) {
        ...
        <Grid container direction="row" sx={{ height: '600px' }}>
            <Grid item xs={3}
                ...
                >
                    ...
                    {['input', 'large_input', 'checkbox', 'radio', 'select', 'submit'].map((type) => (<Box maxWidth={300} key={type}>
                            ...
                            <Collapse in={openEditIndex[type]} timeout="auto" unmountOnExit>
                                <Formik
                                    ...
                                    onSubmit={(values, { setSubmitting }) => {
                                        addFormComponent(type, values.label, uuidv4(), values.placeholder, values.minRows, values.isRequired, values.initialValue, values.defaultChecked);
