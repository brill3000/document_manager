...
class Message {
    constructor(id, from, to, time_sent, message, files) {
        this.messageId = id;
        this.from = from;
        this.to = to;
...
const messageConverter = {
    ...
    fromFirestore: (snapshot, options) => {
        ...
        return new Message(data.from, data.to, data.time_sent, data.message, data.files);
    }
};
            ...
            async queryFn(queryParams) {
                try {
                    ...
                    if (queryParams.to && !queryParams.from) {
                        ...
                        const q = query(
                            collection(db, 'messages'),
                            where('from.id', '==', queryParams.from),
                            orderBy('time_sent', 'desc'),
                            limit(10)
                        );
                        const querySnapshot = await getDocs(q);
                        ...
                        const q2 = query(
                            collection(db, 'messages'),
                            where('from.id', '==', queryParams.to),
                            where('to.id', '==', queryParams.from),
                            orderBy('time_sent', 'desc'),
                            limit(10)
                        );
...
export const { useGetMessageByUserQuery, useSendMessageMutation, useRefetchMessagesMutation } = messageQuery;
