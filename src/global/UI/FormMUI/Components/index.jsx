...
export const FormikTextMaterialMultiline = React.forwardRef(({ name, ...otherProps }, ref) => {
    ...
    return (
        <MateralTextField multiple {...configTextField} ref={ref} />
    )
})
    ...
    return (
        <Stack direction={"column"} spacing={1}>
            <Textarea
                ...
                {...configTextField}
            />
            <Typography level="body2" color="danger">{configTextField.helperText}</Typography>
        </Stack>
...
export const FormikSelect = ({ name, children, ...otherProps }) => {
    ...
    return (
        <TextField
            ...
            select
        >
            {Array.isArray(array) && values.map((option) => (
...
export const FormikAutoComplete = ({ name, ...otherProps }) => {
    ...
    return (
                <Autocomplete
                    ...
                    style={{ width: '100%' }}
                    renderInput={(params) => (
                        <MateralTextField {...params} variant={variant} name={name} label={label} {...configAutoCompleteField} placeholder={placeholder} />
                    )}
                />
    )
}
