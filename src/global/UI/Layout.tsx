        ...
        sx={[
            {
                ...
                gridTemplateColumns: {
                    xs: '1fr',
                    sm: 'minmax(64px, 200px) minmax(450px, 1fr)',
                    md: 'minmax(200px, 300px) minmax(300px, 400px) minmax(600px, 1fr)'
                },
                gridTemplateRows: '64px 1fr',
                minHeight: '100vh'
            },
...
const Department = (props: BoxProps) => (
    <Box
        {...props}
        sx={[
            {
...
const SideDrawer = ({ onClose, ...props }: BoxProps & { onClose: React.MouseEventHandler<HTMLDivElement> }) => (
    <Box
        {...props}
        sx={[{ position: 'fixed', zIndex: 1200, width: '100%', height: '100%' }, ...(Array.isArray(props.sx) ? props.sx : [props.sx])]}
    >
        <Box
            role="button"
            onClick={onClose}
